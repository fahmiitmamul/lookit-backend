...
const { outgoing_guarantee } = require('../models/index')
...
module.exports = {
    getAll: async (req, res) => {
        try {
            ...
            const { count, rows } = await outgoing_guarantee.findAndCountAll({
                where: {
                    [Op.or]: [
                        {
                            file: {
            ...
            return res.json({
                success: true,
                message: 'Get all outgoing guarantee successfully',
                results: {
                    data: rows,
                    totalData: count,
    ...
    create: async (req, res) => {
        try {
            ...
            const data = await outgoing_guarantee.create({
                ...req.body,
            ...
            return res.json({
                success: true,
                message: 'Create outgoing guarantee successfully',
                results: data,
            })
        } catch (err) {
            return errorhandler(res, err)
    ...
    findOneById: async (req, res) => {
        try {
            const data = await outgoing_guarantee.findOne({
                where: {
                    id: req.params.id,
                },
            })
            return res.json({
                success: true,
                message: 'Get outgoing guarantee successfully',
                results: data,
            })
        } catch (err) {
            return errorhandler(res, err)
    ...
    update: async (req, res) => {
        try {
            ...
            const data = await outgoing_guarantee.update(req.body, {
                where: {
                    id: req.params.id,
                },
            })
            ...
            return res.json({
                success: true,
                message: 'Update outgoing guarantee successfully',
                results: data,
            })
        } catch (err) {
            return errorhandler(res, err)
    ...
    delete: async (req, res) => {
        try {
            const data = await outgoing_guarantee.destroy({
                where: {
                    id: req.params.id,
                },
            })
            return res.json({
                success: true,
                message: 'Delete outgoing guarantee successfully',
                results: data,
            })
        } catch (err) {
            return errorhandler(res, err)
