...
const { master_salary_deduction } = require('../models/index')
...
module.exports = {
    getAll: async (req, res) => {
        try {
            ...
            const { count, rows } =
                await master_salary_deduction.findAndCountAll({
                    where: {
    ...
    create: async (req, res) => {
        try {
            const data = await master_salary_deduction.create(req.body)
            return res.json({
                success: true,
    ...
    findOneById: async (req, res) => {
        try {
            const data = await master_salary_deduction.findOne({
                where: {
    ...
    update: async (req, res) => {
        try {
            const data = await master_salary_deduction.update(req.body, {
                where: {
                    master_salary_code: req.params.id,
                },
            })
    ...
    delete: async (req, res) => {
        try {
            const data = await master_salary_deduction.destroy({
                where: {
                    master_salary_code: req.params.id,
                },
            })
